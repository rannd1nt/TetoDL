...
class TestConfigLoad:
    ...
    def test_load_config_defaults(self, monkeypatch, mocker, tetodl_trace):
        ...
        mock_detect = mocker.patch("tetodl.core.domain.config.detect_system_language", return_value="en")
        mock_set = mocker.patch("tetodl.core.domain.config.set_language")
    ...
    def test_load_config_from_file(self, monkeypatch, mocker, tmp_path):
        ...
        mocker.patch("tetodl.core.domain.config.detect_system_language")
        mock_set = mocker.patch("tetodl.core.domain.config.set_language")
...
class TestConfigDeletion:
    ...
    def test_perform_full_wipe(self, monkeypatch, tmp_path, mocker):
        ...
        monkeypatch.setattr(cfg, "CONFIG_PATH", str(config_file))
        monkeypatch.setattr(cfg, "REGISTRY_PATH", str(reg_file))
        mocker.patch("tetodl.core.domain.history.reset_history", return_value=True)
...
class TestConfigHelpers:
    ...
    def test_clear_history(self, mocker):
        ...
        mock_reset = mocker.patch("tetodl.core.domain.history.reset_history", return_value=True)
        from tetodl.core.domain.config import clear_history
    ...
    def test_update_language_valid(self, monkeypatch, tmp_path, mocker):
        ...
        config_file.write_text("{}")
        monkeypatch.setattr(cfg, "CONFIG_PATH", str(config_file))
        mocker.patch("tetodl.core.domain.config.set_language", return_value=True)
    ...
    def test_update_language_invalid(self, monkeypatch, tmp_path, mocker):
        ...
        config_file.write_text("{}")
        monkeypatch.setattr(cfg, "CONFIG_PATH", str(config_file))
        mocker.patch("tetodl.core.domain.config.set_language", return_value=False)
    ...
    def test_get_language_name(self):
        ...
        from tetodl.core.domain.config import get_language_name
