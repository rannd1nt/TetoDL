...
class _DiskCache:
    def __init__(self, namespace: str, default_ttl: float) -> None:
        self._base = Path(CACHE_DIR) / "cache" / namespace
        self._default_ttl = default_ttl
        self._lock = threading.Lock()
...
def get_cache_size() -> str:
    ...
    for ns in _CACHE_CFG:
        d = Path(CACHE_DIR) / "cache" / ns
        if d.is_dir():
            total += sum(
                f.stat().st_size for f in d.glob("*") if f.is_file()
            )
    img_dir = Path(CACHE_DIR) / "cache" / "images"
    if img_dir.is_dir():
...
def evict_cache(max_age: float | None = None) -> int:
    ...
    from tetodl.core.cover.image import evict_img_cache
